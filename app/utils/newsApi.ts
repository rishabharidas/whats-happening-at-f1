...
async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      if (attempt > 0) {
        ...
        serverLog(
          `[NewsAPI] Retrying request to ${url}. Attempt ${attempt} of ${maxRetries} after ${delay}ms...`,
        );
      ...
      if (response.status === 403 || response.status === 401) {
        serverLog(
          `[NewsAPI] Authentication Error ${response.status} on ${url}. Checking keys.`,
        );
        lastResponse = response;
        ...
        try {
          ...
          try {
            ...
            if (parsed.status === "error") {
              isValid = false;
              serverLog(
                `[NewsAPI] Internal error body returned from 200 response: ${parsed.message}`,
              );
            }
          } catch {
            isValid = false;
          ...
          if (isValid) {
            ...
            saveCache(cache);
            serverLog(
              `[NewsAPI] Successfully cached fresh response for ${url}`,
            );
          }
        } catch (e: any) {
          serverLog(
            `[NewsAPI Cache] Failed to cache response: ${e?.message || e}`,
          );
        }
      }
      ...
      serverLog(
        `[NewsAPI] Network error fetching ${url}: ${err?.message || err}`,
      );
      lastError = err;
    }
  }
  ...
  serverLog(
    `[NewsAPI] All ${maxRetries} retries exhausted for ${url}. Error: ${lastError?.message || lastError}`,
  );
...
export default function newsApi() {
  ...
  if (!apiKey) {
    serverLog(
      "[NewsAPI] WARNING: NEXT_NEWS_API_KEY is not defined in environment variables. Falling back to mocks.",
    );
  }
  ...
  return {
    getF1News: async (pageSize = 12) => {
      ...
      // query everything F1
      const params = {
        // Use bare root domains/subdomains only. No paths allowed.
        domains:
          "www.formula1.com,formula1.com,motorsport.com,skysports.com,autosport.com,the-race.com",
        q: 'f1 OR "Formula 1"', // Widens the keyword match safety net
        language: "en",
        sortBy: "publishedAt",
